...
InputFile.prototype.createField = function(field) {
  ...
  btn.innerText = options.buttonText || 'Выберите фото';
  dropArea.insertBefore(btn, field);

  // Create hint element
  var hint = document.createElement('span');
  hint.className = 'inf__hint';
  hint.innerText = options.hint || 'Или перетащите файл в это поле';
  ...
  field.addEventListener('change', function() {
    ...
    if (filesCount === 1) {
      ...
      hint.innerText = filesCount + ' ' + (options.message || 'Фото выбрано');
    }
  });
};
