...
from apps.shop.admin import ProductAdmin
...
class CategoryAttributeAdminForm(forms.ModelForm):
    values = forms.ModelMultipleChoiceField(
        ...
        widget = FilteredSelectMultiple(verbose_name=_("values"), is_stacked=False),
        required = False
    )
...
class ProductAttributesFormSet(forms.BaseInlineFormSet):
    def __init__(self, *args, **kwargs):
        if kwargs['instance'].pk:
            ...
            attributes = CategoryAttribute.objects.filter(parent__in=categories).distinct('attribute').exclude(pk__in=exclude)
