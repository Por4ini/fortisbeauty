...
def createLanguageModels():
    ...
    for model in Translation.__subclasses__():
        ...
        fields = {
            ...
            'translation' : models.BooleanField(default=False, verbose_name=_("Перевести"))
        }

        for field in model._meta.get_fields(include_hidden=False):
            if field.get_internal_type() in ['CharField', 'TextField'] and not any([s in str(field.name).lower() for s in ['json','image','url']]):
                field._unique = False
